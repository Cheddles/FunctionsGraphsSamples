class Axes{
  ...
  void display(){
    ...
    if ((xMin<=0)&&(xMax>=0)){  //check that x=0 is within the drawn range
      ...
      arrowHead(int(map(0,xMin,xMax,0,width)), int(map(yMax,yMin,yMax,height,0)), 0, int(height/(2*ySteps)));
    }
    stroke(yColour);
    ...
    if ((yMin<=0)&&(yMax>=0)){  //check that y=0 is within the drawn range
      ...
      arrowHead(int(map(xMax,xMin,xMax,0,width)), int(map(0,yMin,yMax,height,0)), PI/2, int(width/(2*xSteps)));
    }
    stroke(xColour);
